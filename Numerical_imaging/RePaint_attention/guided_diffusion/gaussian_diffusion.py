...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        model_output, attention_outputs = model(x, self._scale_timesteps(t), **model_kwargs)
    ...
    def p_sample( 
        ...
        #turn each tensor in attention_outputs into attention map
        #if out['attention_outputs'] is not None:
            #attention_outputs = out['attention_outputs']
            #for key in attention_outputs:
             #   attention_outputs[key] = self.get_attention_map(attention_outputs[key], x, idx_wall=idx_wall)
            #out['attention_outputs'] = attention_outputs
        ...
        result = {"sample": sample,
                  "pred_xstart": out["pred_xstart"], 'gt': model_kwargs.get('gt'),'attention_maps': out['attention_outputs']}
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            attention_maps.append(sample['attention_maps'])
        final['attention_maps']=attention_maps
        if return_all:
            return final
        else:
            return final["sample"], attention_maps
