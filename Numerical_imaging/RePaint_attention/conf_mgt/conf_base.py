...
def tensor_to_image(tensor):
    ...
    if array.shape==(1024,8,8):
        array.reshape(256,256,1)
    if array.shape==(1024,16,16):
        array.reshape(256,256,4)
    return array
...
class Default_Conf(NoneDict):
    ...
    def eval_imswrite(self, srs=None, img_names=None, dset=None, name=None, ext='png', lrs=None, gts=None, gt_keep_masks=None,gt_attention=None, verify_same=True):
        ...
        if gt_attention is not None:
            att_dir_path = expanduser(
                self['data'][dset][name]['paths']['gt_attention'])
            
            gt_attention = [tensor_to_image(x[0].cpu()) for x in gt_attention]
            img_att = [str(x) for x in range(len(gt_attention))]
            write_images(gt_attention, img_att, att_dir_path)
